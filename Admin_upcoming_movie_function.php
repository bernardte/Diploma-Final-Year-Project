...
include 'dbh.inc.php';
    ...
    $insert = "INSERT INTO upcoming_movie VALUES('Null', '$name', '$duration','$category', '$description' ,'$img')";
...
// delete execution
if(isset($_GET['id'])){
    $sql = "DELETE FROM upcoming_movie WHERE id = {$_GET['id']}";
    $result = mysqli_query($conn, $sql);
    if($result && mysqli_affected_rows($conn) > 0){
        echo '<script>alert("Deleted successfully"); document.location.href= "Admin_movie_upcoming.php";</script>';
    } else {
        echo '<script>alert("Delete unsuccessfully"); document.location.href= "Admin_movie_upcoming.php";</script>';
    }
}
